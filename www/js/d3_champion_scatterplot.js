...
function setChampData(champ, patch, region, tier){
    var file = "data/by_champion/" + champ + "_" + patch + "_" + region + "_" + tier + "_builds.json";
    loadDataFromFile(
        file,
        function(json){
            updateDataset(json);
...
function getRandomPoints(volume, maxValue){
    ...
    for (var i = 0; i < volume; i++) {
        var newObject = {};
        newObject.x = Math.floor(Math.random() * maxValue);
        newObject.y = Math.floor(Math.random() * maxValue);
